...
TBuffer::TBuffer(size_t initSize, size_t minExpandSize)
    : mInitSize(initSize), mMinExpandSize(minExpandSize)
{
    mSize = 0;
    mCapacity = initSize;
...
TBuffer::TBuffer(const TBuffer& rhs)
    : mInitSize(rhs.mInitSize), mMinExpandSize(rhs.mMinExpandSize)
    , mSize(rhs.mSize), mCapacity(rhs.mCapacity)
{
    mBuffer = (uint8_t*)malloc(mCapacity * sizeof(uint8_t));
